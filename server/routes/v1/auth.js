...
router.post('/register', (req, res) => {
  ...
  ).run(username, hash, encEmail, emailHash, 0, rid, 'owner');
...
router.post('/verify-and-login', (req, res) => {
...
router.post('/send-code', (req, res) => {
  ...
  const emailHash = hashEmail(email);
  const existing = db.prepare('SELECT id FROM owners WHERE email_hash = ?').get(emailHash);
